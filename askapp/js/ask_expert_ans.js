...
/*if($sBack[0]){
    $sBack[0].href = 'ask_expert_ans.html?'+getExpertId;
}*/
if($newAskLink[0]){
    $newAskLink[0].href = 'ask_expert_q.html?'+getExpertId;
}
...
//var data = {'page':1,'expertid':'12040614042162f2385e','flag':2};
...
function getdata(){
  ...
  if(navigator.onLine){
    ...
    ajax.run({'url': config.expertBestAnsUrl, 'type':'get'},data,setList);
  }else{
    alert('网络连接不上，请检查');
  }
};
...
var setdata =function(r){
  ...
  var qData = r.data,
    ...
    $btnQuiz.on('click',function(){
        ...
        //$(this)[0].href = 'ask_expert_submit.html?' + getExpertId +'&' + qExpertName;
    });
    /*if($btnQuiz[0]){
